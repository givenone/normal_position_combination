...
for i in $SET
    do
        ...
        ./mesh_opt "before_correction/output$i $j.ply" -lambda 0.8 "norm:after_correction_optimized/result$i $j.ply"
        # lambda : default 0.1, weight for corrected normal.
    done
done

python mesh_merge.py after_correction/result final_binary_optimized_0.8.ply